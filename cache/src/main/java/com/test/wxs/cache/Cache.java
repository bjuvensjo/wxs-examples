...
public interface Cache<K extends CacheKey, V extends Serializable> {
    ...
    /**
     * Returns the value in the cache which is identified by the specified key, or null if no such value
     * exists.
     * 
     * @param key the key
     * @return the value in the cache which is identified by the specified key, or null if no such value
     *         exists
     */
    V get(K key);

    /**
     * Returns the value(s) in the cache that are identified by the specified keys.
     * If a key in the list cannot be found, a null value will be set at the corresponding position in the returned list.
     * 
